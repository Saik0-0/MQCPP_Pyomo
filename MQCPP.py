...
model.edges_3 = pe.Set(dimen=2, initialize={(u, v) if u < v else (u, u) for u in V for v in V})
model.edges_triples = pe.Set(dimen=3, initialize={(u, v, v_1) for u in V for v in V for v_1 in V if u <= v < v_1})
model.edges_triples_2 = pe.Set(dimen=3, initialize={(u, v, v_1) for u in V for v in V for v_1 in V if u <= v < v_1 and (v, v_1) in E})
...
model.twenty_five_constraint = pe.Constraint(model.i, rule=twenty_five_constraint_func)
...
model.thirty_eight_constraint = pe.Constraint(model.edges_triples_2, rule=thirty_eight_constraint_func)
